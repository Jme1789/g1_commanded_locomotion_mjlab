    ...
    std::shared_ptr<unitree::common::UnitreeJoystick> joystick)
{
  ...
  if (m->nu > NUM_MOTOR_IDL_GO) {
    interface = std::make_unique<G1Bridge>(m, d, joystick);
  } else {
    interface = std::make_unique<Go2Bridge>(m, d, joystick);
  }
  interface->start();
...
int main(int argc, char **argv)
{
  ...
  if(param::config.gamepad_config.is_relative()) {
    param::config.gamepad_config = proj_dir / param::config.gamepad_config;
  }

  std::shared_ptr<unitree::common::UnitreeJoystick> joystick;
  if (param::config.use_joystick == 1) {
    try {
      joystick = gamepad::ConfiguredJoystick::Create(
          param::config.gamepad_config);
    } catch (const gamepad::GamepadConfigError& error) {
      std::cerr << "Gamepad startup failed for active path "
                << param::config.gamepad_config << ": " << error.what()
                << std::endl;
      return EXIT_FAILURE;
    }
  }
  ...
  std::thread unitree_thread(UnitreeSdk2BridgeThread, joystick);
