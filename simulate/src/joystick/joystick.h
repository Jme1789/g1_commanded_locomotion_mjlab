...
public:
  ...
  using ReadOperation = std::function<ssize_t(int, void *, std::size_t)>;
...
private:
  ...
  bool owns_fd_{true};
  int last_error_{0};
  ReadOperation read_operation_{
      [](int fd, void *destination, std::size_t size)
      {
...
public:
  ...
  /**
   * Injects the read boundary for deterministic reader error tests.
   */
  Joystick(int fd, bool owns_fd, ReadOperation read_operation);
   ...
   * from the joystick. Returns true if one complete event is available.
   */
  bool sample(JoystickEvent *event);
  ReadStatus readEvent(JoystickEvent *event);
  int lastError() const;
};
