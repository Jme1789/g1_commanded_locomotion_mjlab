...
public:
    UnitreeSDK2BridgeBase(
        mjModel *model,
        mjData *data,
        std::shared_ptr<unitree::common::UnitreeJoystick> injected_joystick)
    : mj_data_(data), mj_model_(model), joystick(std::move(injected_joystick))
    {
        _check_sensor();
...
public:
    ...
    : RobotBridge(model, data, std::move(joystick))
    {
