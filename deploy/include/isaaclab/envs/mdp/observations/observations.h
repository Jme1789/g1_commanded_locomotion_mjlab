...
{
    const auto command = env->velocity_command();
...
REGISTER_OBSERVATION(swing_height_command)
{
    return std::vector<float>{env->swing_height_command_m()};
}

REGISTER_OBSERVATION(step_length_command)
{
    return std::vector<float>{env->step_length_command_m()};
}
