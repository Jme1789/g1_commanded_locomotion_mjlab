...
std::unique_ptr<CustomJoystick> FSMState::custom_joystick = nullptr;
...
int main(int argc, char** argv)
{
    ...
    if (!param::custom_joystick.empty()) {
        try {
            FSMState::custom_joystick =
                std::make_unique<CustomJoystick>(param::custom_joystick);
        } catch (const std::exception& exception) {
            spdlog::critical("Cannot start custom joystick input: {}",
                             exception.what());
            return 1;
        }
    }
    ...
    std::cout << "A confirmed fall in Velocity enters damping protection.\n";
    std::cout << "In Fallen, release [A], then hold it for one second to enter GetUp mode.\n";
