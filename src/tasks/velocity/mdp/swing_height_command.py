...
class DiscreteSwingHeightCommand(CommandTerm):
  """Sample one scalar swing-foot height from a fixed set of levels."""

  cfg: DiscreteSwingHeightCommandCfg

  def __init__(
    self, cfg: DiscreteSwingHeightCommandCfg, env: ManagerBasedRlEnv
  ) -> None:
    super().__init__(cfg, env)
    self.height_command = torch.zeros(self.num_envs, 1, device=self.device)
    self._levels = torch.tensor(
      cfg.levels_m, device=self.device, dtype=torch.float32
    )
    self._probabilities = torch.tensor(
      cfg.probabilities, device=self.device, dtype=torch.float32
    )
  ...
  def _resample_command(self, env_ids: torch.Tensor) -> None:
    ...
    self.height_command[env_ids, 0] = self._levels[level_indices]
  ...
  def _log_transition(
    ...
    print(
      f"[SWING_HEIGHT] level={label:<4} target={target_m:.3f} m",
      flush=True,
    )
...
class DiscreteSwingHeightCommandCfg(CommandTermCfg):
  """Configuration for a discrete physical swing-height command."""
  ...
  def __post_init__(self) -> None:
    ...
    if any(
      ...
      for current, following in zip(
        self.levels_m, self.levels_m[1:], strict=False
      )
    ):
    ...
    if not all(
      math.isfinite(value) and value >= 0.0 for value in self.probabilities
    ):
    ...
    if not (
      ...
      raise ValueError(
        "resampling_time_range must be finite, positive, and ordered"
      )

  def build(self, env: ManagerBasedRlEnv) -> DiscreteSwingHeightCommand:
    return DiscreteSwingHeightCommand(self, env)
