...
if __name__ == "__main__":
  # Keep TrainConfig identity stable for callers that import scripts.train.
...
RL_TUNING: dict[str, dict[str, TuningEntry]] = {
  "training_runtime": {
    ...
    "print_effective_config": _tuning_entry(
      "打印最终生效配置并退出。", "布尔值", "用于训练前核对默认值、字典值和 CLI 覆盖。", "由训练命令行入口处理。", requires_retraining=False
    ),
    # 预留：把最终生效配置导出为 YAML。
    "dump_effective_config": _tuning_entry(
      "最终生效配置导出路径。", "YAML 文件路径或 None", "便于保存和比较每次调参结果。", "由训练命令行入口处理。", requires_retraining=False
    ),
...
def allocate_training_log_dir(
  ...
  *,
  ...
  timestamp = datetime.now(tz=UTC).astimezone() if now is None else now
...
def launch_training(
  ...
  # Allocate once before launching workers; external callers may own this path.
  if log_dir is None:
